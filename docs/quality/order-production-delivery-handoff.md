...
| 个人中心、附加手机号表单 | 业务请求和状态边界已闭合 | 真机复现表单被压缩、按钮行未占满 | **未完成** |
...
真机尺寸已复现：附加手机号标题逐字换行，手机号、姓名和保存按钮被挤到同一窄行；商户登录与联系客服两个原生 `button` 行只占卡片中部，不能作为可交付 UI。

- [ ] 为附加手机号表单建立独立的全宽布局；标题、已保存号码、手机号、姓名和保存操作不得相互挤压。
- [ ] 输入控件具有明确最小宽度、可点击高度和焦点态；小屏不得逐字换行。
- [ ] `.merchant-login` 与客服 `button` 行宽度为卡片可用宽度，清除原生 button 默认 margin/padding/伪边框影响。
- [ ] 在常用 iPhone 小屏和标准屏分别验证，不以单一模拟器截图替代。
- [ ] 增加能断言实际 bounding box、行宽和无逐字换行的 rendered UI 回归；现有“节点存在”测试不足以捕获此问题。

完成证据：Red→Green、真机/官方开发者工具截图、关键控件 bounding box receipt、Mini 默认 Gate。
...
- [ ] P0-1 至 P0-6 全部关闭且无 P0/P1 finding。
