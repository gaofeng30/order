...
func TestCatalogSchemaIntegration(t *testing.T) {
	withCatalogSchema(t, func(db *sql.DB) {
		...
		if len(migrationSet) != 7 {
			t.Fatalf("migration count = %d, want 7", len(migrationSet))
		}
		...
		if err != nil || catalogResult.FromVersion != 1 || catalogResult.ToVersion != 7 || catalogResult.AppliedCount != 6 {
			t.Fatal("catalog migrations did not advance version 1 to version 7")
		}
		...
		if err != nil || repeat.FromVersion != 7 || repeat.ToVersion != 7 || repeat.AppliedCount != 0 {
