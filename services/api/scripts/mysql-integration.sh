...
for variable in "${required[@]}"; do
  if [[ -z "${!variable:-}" ]]; then
    printf 'mysql integration environment is incomplete: %s\n' "${variable}" >&2
    exit 1
  fi
done

if [[ "${ORDER_TEST_MYSQL_INSTANCE}" != "order-mysql-w3" || "${ORDER_TEST_MYSQL_ISOLATED}" != "YES" ]]; then
  printf 'mysql integration environment ownership mismatch\n' >&2
  exit 1
fi

GOTOOLCHAIN="${GOTOOLCHAIN:-go1.26.5}" GOPROXY="${GOPROXY:-off}" \
  go test -race ./services/api/internal/migrate -run '^TestMySQL8Integration$' -count=1 -timeout=3m
