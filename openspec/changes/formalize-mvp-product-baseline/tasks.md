...
- [x] 2.7 把 §13 改为生产配置与外部依赖，按 design D7 写入固定 12 Gate、责任方、非敏感证据引用、阻塞阶段和更新时间字段；将真实商品/库存/时段/截单/取餐点/员工名单/价格值明确为 UAT 前配置。
  - Verifier Red：对旧候选 `04b05fda843ca0f1b02764a39f50b0068db0141d` 解析 §13.2，检查器返回 `EXTERNAL_GATE_MISSING_UPDATED_AT`；表头只有 Gate、外部依赖、责任方、初始状态、仅阻塞阶段、非敏感完成证据 6 列，与 spec 要求的更新时间字段不一致。
  - Green：Python 解析 §13.2 表格，确认表头含“更新时间”、Gate 1–12 共 12 行均有非空值 `2026-08-12（基线建立）`，且 12 行状态仍全部为`BLOCKED_EXTERNAL`；该日期仅表示本 PRD 基线记录更新时间，不表示外部依赖完成。敏感数据静态检查 PASS。
