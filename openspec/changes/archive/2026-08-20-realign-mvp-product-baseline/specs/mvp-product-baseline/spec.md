...
**Reason**: 2026-08-19 客户评审记录第 3 条明确一期不做数量库存、库存预占和自动扣减，商品只有可售与售罄两种销售状态。商品可售性改由新增的 `Product availability uses a per-service-date sellout switch` 承担。

**Migration**: 无运行态数据需要迁移——该 requirement 从未进入实现，`services/api` 仅有菜品目录能力。后续 change MUST NOT 引入份数、剩余量、预占或自动扣减；相关前端展示位按 0818 PRD §15.5 删除。

### Requirement: Order submission uses a bounded atomic soft hold

**Reason**: 2026-08-19 客户评审记录第 3 条与第 30 条删除库存预占与迟到支付流程，并规定支付取消或失败不生成订单。下单与支付路径改由新增的 `Orders exist only after confirmed WeChat payment` 与 `Unmatched payments are reconciled into orders or manual handling` 承担。

**Migration**: 无运行态数据需要迁移。`待支付` 状态与 15 分钟超时释放一并取消；「用户已支付但订单未生成」的资损风险由对账兜底 requirement 覆盖，MUST NOT 退回迟到支付与重占设计。

### Requirement: Orders use one nine-state production state machine

**Reason**: 2026-08-19 客户评审记录第 4、19、20、21 条删除接单、`已取消`、异常与待取超时，第 30 条规定支付前不生成订单。状态集合改由新增的 `Orders use one six-state production state machine` 承担。

**Migration**: 无运行态数据需要迁移。前端 `apps/wechat-miniprogram/utils/util.js` 的三态推进与撤销交互按新 requirement 改造，禁止撤销的约束在六态 requirement 中原样保留。

### Requirement: Employee price is an optional fixed per-product amount

**Reason**: 2026-08-19 客户评审记录第 8 条改为 PC 后台维护手机号白名单与整单折扣，员工价在前端以划线原价形式展示。定价改由新增的 `Employee pricing uses one global discount rate applied per product` 承担。

**Migration**: 无运行态数据需要迁移。商品模型 MUST 移除 `employee_price` 字段；已生成订单的价格快照不回算，但一期尚无生产订单。

### Requirement: Every first-phase order uses one fixed pickup slot

**Reason**: 2026-08-19 客户评审记录第 6、7、13 条与评审后澄清第 10、11 条把取餐时段改为离散取餐时间点，并把可预约日期限定为今天与明天。取餐模型改由新增的 `Every first-phase order uses one discrete pickup time` 承担。

**Migration**: 无运行态数据需要迁移。「固定取餐时段」与「餐段空档期」两个概念一并取消；截单时刻按餐段固定的约束在新 requirement 中原样保留。

### Requirement: Merchant permissions use four server-enforced roles

**Reason**: 2026-08-19 客户评审记录第 26 条与第 32 条把商户角色改为主账号与子账号两个，并要求商户账号名单与折扣白名单分开维护。权限模型改由新增的 `Merchant permissions use two server-enforced roles` 承担。

**Migration**: 无运行态数据需要迁移。店管、后厨、核销、财务只读四个角色不再存在；服务端执行鉴权与最小权限脱敏的约束在新 requirement 中原样保留。
