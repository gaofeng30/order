...
- 将根治理与 `order-run-loop` 精简接入该入口，保留现有 stage skills、exact-SHA、独立验证、两 lane 和授权边界。
...
- owned paths：
  ...
  - `AGENTS.md`
- 只读共享契约：四个 `$order-*` stage skills、`docs/quality/change-quality-gates.md`、生命周期 receipt 工具与历史归档。
...
- 非目标：引入数据库、服务、daemon、CI、第三方包或图编排框架；自动批准、自动集成、自动推送、自动部署；替代 OpenSpec、Git、独立 verifier 或 receipt；修复 README/业务功能；归档三个 legacy change。
