...
async function verify(path, data) {
  const body = await api.intrinsic(path, data);
  return orderStore.parseDetail(body && body.order);
}
function verifyScan(token) { return verify('/api/v1/verify/scan', { token }); }
function verifyCode(pickupNumber) { return verify('/api/v1/verify/code', { pickup_number: pickupNumber }); }
