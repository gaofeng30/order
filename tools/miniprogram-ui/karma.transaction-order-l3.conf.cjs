...
module.exports = function configure(config) {
  ...
  config.set({
    ...
    preprocessors: {
      'tools/miniprogram-ui/test/browser/ui1-transaction-order-l3.spec.cjs': ['webpack'],
    },
    webpack: {
      mode: 'development',
      devtool: false,
      ...
      plugins: [
        new webpack.DefinePlugin({
          ORDER_TRANSACTION_L3_PROXY_ORIGIN: JSON.stringify(process.env.ORDER_TRANSACTION_L3_PROXY_ORIGIN),
          ORDER_TRANSACTION_L3_FIXTURE: process.env.ORDER_TRANSACTION_L3_FIXTURE,
        }),
        new webpack.NormalModuleReplacementPlugin(
          /runtimeEndpointConfig\.js$/,
          path.join(__dirname, 'test/browser/transaction-order-l3-runtime-endpoint-config.cjs'),
        ),
      ],
    },
    reporters: ['progress'],
    browsers: ['OrderTransactionL3ChromiumHeadless'],
    customLaunchers: {
      OrderTransactionL3ChromiumHeadless: {
        base: 'ChromeHeadless',
