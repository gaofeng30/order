...
The composed runner does not start MySQL or claim an API port. Its default customer and pending flows do not perform test setup. It opens a random loopback CORS proxy, forwards request methods, paths, headers, and bodies unchanged, and reports upstream status codes plus sanitized request IDs. Its HTTP-failure scenario rewrites only the request path to a guaranteed-absent route; the resulting `404` is returned by the real `order-api`, not fabricated by the runner.
...
The merchant flow is a separate one-scenario gate against the default development auto-pay provider:

```sh
ORDER_COMPOSED_FLOW=merchant \
ORDER_COMPOSED_API_ORIGIN=http://127.0.0.1:8080 \
MINIPROGRAM_UI_DEPS=/path/to/reused/tools/miniprogram-ui \
npm --prefix tools/miniprogram-ui run ui1:composed
```

It obtains a real OWNER PC session, saves `/api/v1/admin/settings`, and uses the public Admin API to open today's service date with one pickup approximately 20–25 minutes ahead. The rendered Mini Program then creates a `PREPARING` order, enters through merchant phone login, writes and restores the store status, marks the order ready, reads the server redemption token from the user detail, atomically scans it completed, and toggles the selected date's sold-out fact true then false. A runner `finally` always restores that product's baseline sold-out fact and the saved Admin settings through authenticated APIs; cleanup failure fails the gate. No raw SQL is used.
...
The merchant flow still fakes only the two unavailable native seams: development `wx.requestPayment` success and `wx.scanCode` returning the server-issued token. It does not prove real payment, funds, camera scanning, WeChat DevTools UI2, or physical-device UI3.
