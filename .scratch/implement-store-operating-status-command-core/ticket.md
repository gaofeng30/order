...
- status: `REPLACEMENT_CANDIDATE`; exact replacement SHA is recorded externally after the self-containing commit
...
- invalidated candidate SHA: `25152cd98bc76f8bcb87dc42473ca4cbd55755e8`; replacement SHA `RECORDED_EXTERNALLY` and is bound only after the final owned-only commit
- gate: `W3`
...
Create `services/api/internal/storestatus/**` as the sole production mutation seam for `storefront_settings.business_status`. The Module accepts DB, `merchantidentity.Authorizer` and a clock at construction, then hides live RBAC, singleton locking, replay/conflict, exact single-column mutation, durable success audit, raw deadlock/lock-timeout retry, folded authorization-unavailable retry and rollback behind one `Apply` method.

Minimum success is all frozen slices in `spec.md` Green against real MySQL, all eleven required mutants killed, full Writer Gate passing, an owned-only Chinese replacement commit and a clean worktree. Review, detached verification and integration restart on the replacement and remain controller-owned.
...
3. Real authorization adapter: `merchantidentity.NewRepository(db).AuthorizeInTx` over live merchant-account rows.
   Its read-only contract folds an account-row SQL failure into `merchantidentity.ErrUnavailable`; Core owns the one-time full transaction retry without modifying that adapter.
...
- Formal Spec P1 has invalidated `25152cd98bc76f8bcb87dc42473ca4cbd55755e8`; no Standards, Spec or verifier result from that SHA may be reused.
...
The disposable loopback Docker MySQL fixture is cleaned after each Gate. Before integration, rollback is deletion of this branch/worktree by the controller. There is no migration or external state to roll back. Raw MySQL `1213`/`1205` and the Authorizer's folded `merchantidentity.ErrUnavailable` are retried through one complete new transaction only; a repeated folded unavailable and every non-retryable error fail closed.
