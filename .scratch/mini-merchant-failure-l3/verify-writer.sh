...
expected_base="c5ab765b82f6869e6fb2176b2adb812a81b925d3"
...
api_port=${ORDER_MERCHANT_FAILURE_API_PORT:?ORDER_MERCHANT_FAILURE_API_PORT is required}
dependency_root=${MINIPROGRAM_UI_DEPS:?MINIPROGRAM_UI_DEPS is required}
database="order_merchant_failure_$(date +%s)_$$"
build_root=$(mktemp -d /private/tmp/order-merchant-failure-build.XXXXXX)
evidence_root="/private/tmp/order-merchant-failure-${candidate_sha:0:12}-$(date +%s)-$$"
...
[[ "${database}" =~ ^order_merchant_failure_[0-9_]+$ ]]
...
unexpected_paths=$(git diff --name-only "${expected_base}...${candidate_sha}" | \
  rg -v '^(apps/wechat-miniprogram/pages/(admin-orders|admin-order-detail|admin-products)/|tools/miniprogram-ui/(karma\.composed-merchant-failure\.conf\.cjs|run-ui1-composed-merchant-failure\.mjs|test/browser/(composed-merchant-failure-runtime-endpoint-config\.cjs|ui1-composed-merchant-failure\.spec\.cjs))|\.scratch/mini-merchant-failure-l3/(miniprogram-gates\.json|verify-writer\.sh))$' || true)
...
node --check tools/miniprogram-ui/run-ui1-composed-merchant-failure.mjs
node --check tools/miniprogram-ui/test/browser/ui1-composed-merchant-failure.spec.cjs
node --check tools/miniprogram-ui/test/browser/composed-merchant-failure-runtime-endpoint-config.cjs
node --check tools/miniprogram-ui/karma.composed-merchant-failure.conf.cjs
node -e 'JSON.parse(require("node:fs").readFileSync(".scratch/mini-merchant-failure-l3/miniprogram-gates.json","utf8"))'
...
ORDER_MERCHANT_FAILURE_FRESH_DB=YES \
ORDER_MERCHANT_FAILURE_API_ORIGIN="http://127.0.0.1:${api_port}" \
ORDER_MERCHANT_FAILURE_MYSQL_CONTAINER="${mysql_container}" \
ORDER_MERCHANT_FAILURE_MYSQL_DATABASE="${database}" \
ORDER_MERCHANT_FAILURE_MYSQL_USER="${mysql_user}" \
ORDER_MERCHANT_FAILURE_MYSQL_PASSWORD="${mysql_password}" \
ORDER_MERCHANT_FAILURE_RECEIPT_PATH="${receipt_path}" \
MINIPROGRAM_UI_DEPS="${dependency_root}" \
PLAYWRIGHT_BROWSERS_PATH=0 \
  node tools/miniprogram-ui/run-ui1-composed-merchant-failure.mjs

node -e 'const fs=require("node:fs");const r=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));if(r.status!=="PASS"||r.candidate_sha!==process.argv[2]||r.cases.join(",")!=="PAGE-M02,PAGE-M03,PAGE-M05"||r.database_schema_version!==44)process.exit(1)' \
