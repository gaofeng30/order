...
mode=${1:-}
case "${mode}" in
  user-pages|staff-profile|detail-media|merchant-pages|merchant-failure) ;;
  *) printf 'usage: %s user-pages|staff-profile|detail-media|merchant-pages|merchant-failure\n' "$0" >&2; exit 64 ;;
esac
...
schema_mode=${mode//-/_}
database="order_${schema_mode}_$(date +%s)_$$"
build_root=$(mktemp -d /private/tmp/order-final-ledger-build.XXXXXX)
evidence_root="/private/tmp/order-final-ledger-${candidate_sha:0:12}-${schema_mode}-$(date +%s)-$$"
...
[[ "${mysql_container}" == order-mysql-w3 ]]
[[ "${mysql_port}" =~ ^[1-9][0-9]{0,4}$ ]]
[[ "${database}" =~ ^order_[a-z_]+_[0-9]+_[0-9]+$ ]]
...
GOCACHE=/Users/vivix/Library/Caches/go-build GOMODCACHE=/Users/vivix/go/pkg/mod \
  GOTOOLCHAIN=go1.26.5 GOPROXY=off go build -o "${migrate_binary}" ./services/api/cmd/order-migrate
GOCACHE=/Users/vivix/Library/Caches/go-build GOMODCACHE=/Users/vivix/go/pkg/mod \
  GOTOOLCHAIN=go1.26.5 GOPROXY=off go build -o "${bootstrap_binary}" ./services/api/cmd/order-bootstrap
GOCACHE=/Users/vivix/Library/Caches/go-build GOMODCACHE=/Users/vivix/go/pkg/mod \
  GOTOOLCHAIN=go1.26.5 GOPROXY=off go build -o "${api_binary}" ./services/api/cmd/order-api
...
case "${mode}" in
  ...
  merchant-pages)
    env "${common_env[@]}" \
      "ORDER_MERCHANT_CLOSURE_API_ORIGIN=http://127.0.0.1:${api_port}" \
      "ORDER_MERCHANT_CLOSURE_MYSQL_CONTAINER=${mysql_container}" \
      "ORDER_MERCHANT_CLOSURE_MYSQL_DATABASE=${database}" \
      "ORDER_MERCHANT_CLOSURE_MYSQL_USER=${mysql_user}" \
      "ORDER_MERCHANT_CLOSURE_MYSQL_PASSWORD=${mysql_password}" \
      "ORDER_MERCHANT_CLOSURE_RECEIPT_PATH=${receipt_path}" \
...
node -e 'const fs=require("node:fs");const r=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));if(r.status!=="PASS"||r.candidate_sha!==process.argv[2])process.exit(1)' \
...
printf 'FINAL_FRESH_ROOT_UI1=PASS mode=%s candidate=%s receipt=%s cleanup=0\n' "${mode}" "${candidate_sha}" "${receipt_path}"
