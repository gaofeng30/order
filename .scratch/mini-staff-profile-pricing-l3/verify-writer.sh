...
expected_base="53e7f427a978e1ed737c8965b76239647ecd523e"
...
api_port=${ORDER_STAFF_PROFILE_API_PORT:?ORDER_STAFF_PROFILE_API_PORT is required}
dependency_root=${MINIPROGRAM_UI_DEPS:?MINIPROGRAM_UI_DEPS is required}
database="order_staff_profile_$(date +%s)_$$"
build_root=$(mktemp -d /private/tmp/order-mini-staff-profile-build.XXXXXX)
evidence_root="/private/tmp/order-mini-staff-profile-${candidate_sha:0:12}-$(date +%s)-$$"
...
[[ "${database}" =~ ^order_staff_profile_[0-9_]+$ ]]
...
unexpected_paths=$(git diff --name-only "${expected_base}...${candidate_sha}" | \
  rg -v '^(apps/wechat-miniprogram/(pages/(menu/menu\.(js|wxml)|detail/detail\.(js|wxml)|profile/profile\.(js|wxml|wxss))|tests/(overnight-menu-detail-ui0|public-read-menu-ui0)\.test\.js)|tools/miniprogram-ui/(karma\.(staff-profile-static|composed-staff-profile)\.conf\.cjs|run-ui1-composed-staff-profile\.mjs|test/browser/(composed-staff-profile-runtime-endpoint-config\.cjs|ui1-(staff-profile-static|composed-staff-profile)\.spec\.cjs))|\.scratch/mini-staff-profile-pricing-l3/(miniprogram-gates\.json|verify-writer\.sh))$' || true)
...
node --check tools/miniprogram-ui/run-ui1-composed-staff-profile.mjs
node --check tools/miniprogram-ui/test/browser/ui1-composed-staff-profile.spec.cjs
node --check tools/miniprogram-ui/test/browser/ui1-staff-profile-static.spec.cjs
node --check tools/miniprogram-ui/test/browser/composed-staff-profile-runtime-endpoint-config.cjs
node --check tools/miniprogram-ui/karma.composed-staff-profile.conf.cjs
node --check tools/miniprogram-ui/karma.staff-profile-static.conf.cjs
...
PLAYWRIGHT_BROWSERS_PATH=0 \
  "${dependency_root}/node_modules/.bin/karma" start tools/miniprogram-ui/karma.staff-profile-static.conf.cjs --single-run --no-colors
...
ORDER_STAFF_PROFILE_FRESH_DB=YES \
...
ORDER_STAFF_PROFILE_RECEIPT_PATH="${receipt_path}" \
MINIPROGRAM_UI_DEPS="${dependency_root}" \
PLAYWRIGHT_BROWSERS_PATH=0 \
  node tools/miniprogram-ui/run-ui1-composed-staff-profile.mjs

node -e 'const fs=require("node:fs");const r=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));if(r.status!=="PASS"||r.candidate_sha!==process.argv[2]||r.cases.join(",")!=="PAGE-U03,PAGE-U09,AC-04,AC-05"||r.database_schema_version!==44)process.exit(1)' \
