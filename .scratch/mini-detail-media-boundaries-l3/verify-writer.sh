...
expected_base="734ce4933fff04f4d73c348950c2d47a81f8d782"
...
api_port=${ORDER_DETAIL_MEDIA_API_PORT:?ORDER_DETAIL_MEDIA_API_PORT is required}
dependency_root=${MINIPROGRAM_UI_DEPS:?MINIPROGRAM_UI_DEPS is required}
database="order_detail_media_$(date +%s)_$$"
build_root=$(mktemp -d /private/tmp/order-mini-detail-media-build.XXXXXX)
evidence_root="/private/tmp/order-mini-detail-media-${candidate_sha:0:12}-$(date +%s)-$$"
...
[[ "${database}" =~ ^order_detail_media_[0-9_]+$ ]]
...
unexpected_paths=$(git diff --name-only "${expected_base}...${candidate_sha}" | \
  rg -v '^(apps/wechat-miniprogram/pages/(menu/menu\.wxml|detail/detail\.wxml)|tools/miniprogram-ui/(karma\.(detail-media-static|composed-detail-media)\.conf\.cjs|run-ui1-composed-detail-media\.mjs|test/browser/(composed-detail-media-runtime-endpoint-config\.cjs|ui1-(detail-media-static|composed-detail-media)\.spec\.cjs))|\.scratch/mini-detail-media-boundaries-l3/(miniprogram-gates\.json|verify-writer\.sh))$' || true)
...
node --check tools/miniprogram-ui/run-ui1-composed-detail-media.mjs
node --check tools/miniprogram-ui/test/browser/ui1-composed-detail-media.spec.cjs
node --check tools/miniprogram-ui/test/browser/ui1-detail-media-static.spec.cjs
node --check tools/miniprogram-ui/test/browser/composed-detail-media-runtime-endpoint-config.cjs
node --check tools/miniprogram-ui/karma.composed-detail-media.conf.cjs
node --check tools/miniprogram-ui/karma.detail-media-static.conf.cjs
rg -q 'font-family: var\(--font-head\)' apps/wechat-miniprogram/components/imageph/imageph.wxss

CHROME_BIN=$(node -e 'const{createRequire}=require("node:module");const path=require("node:path");const r=createRequire(path.join(process.argv[1],"package.json"));process.stdout.write(r("playwright").chromium.executablePath())' "${dependency_root}") \
MINIPROGRAM_UI_DEPS="${dependency_root}" \
PLAYWRIGHT_BROWSERS_PATH=0 \
  "${dependency_root}/node_modules/.bin/karma" start tools/miniprogram-ui/karma.detail-media-static.conf.cjs --single-run --no-colors
...
ORDER_DETAIL_MEDIA_FRESH_DB=YES \
ORDER_COMPOSED_API_ORIGIN="http://127.0.0.1:${api_port}" \
ORDER_DETAIL_MEDIA_RECEIPT_PATH="${receipt_path}" \
MINIPROGRAM_UI_DEPS="${dependency_root}" \
PLAYWRIGHT_BROWSERS_PATH=0 \
  node tools/miniprogram-ui/run-ui1-composed-detail-media.mjs

node -e 'const fs=require("node:fs");const r=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));if(r.status!=="PASS"||r.candidate_sha!==process.argv[2]||r.cases.join(",")!=="PAGE-U04,BE-34,BE-35"||r.database_schema_version!==44)process.exit(1)' \
...
printf 'MINI_DETAIL_MEDIA_WRITER_GATE %s\n' "$(node -e 'const fs=require("node:fs");const r=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));process.stdout.write(JSON.stringify({status:r.status,candidate_sha:r.candidate_sha,cases:r.cases,browser:r.browser,evidence_level:r.evidence_level,receipt:process.argv[1],cleanup:{api_stopped:true,database_dropped:true}}))' "${receipt_path}")"
